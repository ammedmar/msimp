...
\]
The functor induced by the diagonal restriction has a right adjoint $\mathcal{N}^{(k)}$, the multisimplicial nerve of a simplicial set, making the categories of $k$-fold multisimplicial and simplicial sets equivalent in Quillen's sense.
Furthermore, there is a natural inclusion
\[
\In \colon \chains(Y) \to \chains(\mathcal{N}^{(k)} Y)
\]
...
\[
\con{r}(\R) \subseteq \con{r}(\R^2) \subseteq \dotsb 
\]
of the space $\con{r}(\R^{\infty})$ of $r$ distinct ordered points in $\R^\infty \defeq \colim (\R^1 \subseteq \R^2 \subseteq \dotsb)$.
Concretely, for any integer $r$, McClure and Smith \cite{mcclure2003multivariable} introduced a chain complex $\cX(r)$ of $\Z[\Sigma_r]$-modules with a filtration
\[
\cX_{1}(r) \subseteq \cX_{2}(r) \subseteq \dotsb \,,
\]
and showed that $\cX(r)$ is connected to the singular chains of $\con{r}(\R^{\infty})$ via a zig-zag of filtration preserving $\Sigma_r$-equivariant quasi-isomorphisms.
...
By the constructions in \cref{s:multisimplicial} the complex $\chains\sur(r)$ is equipped with an $E_\infty$-coalgebra structure, which we connect to the singular chains of $\con{r}(\R^{\infty})$ via an explicit zig-zag of filtration preserving $\Sigma_r$-equivariant quasi-isomorphisms of $E_\infty$-coalgebras.

In a similar way, Berger and Fresse \cite{berger2004combinatorial} studied a chain complex $\cE(r)$ of $\Z[\Sigma_r]$-modules with a filtration
\[
...
\]
introduced by Smith \cite{smith1989filtration}.\footnote{As before we disregard the operadic structure on $\set{\BE_{d}(r)}_{r \geq 1}$.}
Since $\cE(r)$ is induced from a simplicial set, it is endowed with an $E_\infty$-coalgebra structure, and it is not hard to see that the zig-zag of filtration preserving $\Sigma_r$-equivariant quasi-isomorphisms used to compare it to the singular chains of $\con{r}(\R^{\infty})$ respects this higher structure.
...
Berger--Fresse constructed two such filtration preserving $\Sigma_r$-equivariant quasi-isomorphisms
\[
...
\]
where the second map is induced from a filtration preserving $\Sigma_r$-equivariant weak-equivalence of simplicial sets.
