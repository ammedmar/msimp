...
The functor $(-)^\diag \colon \mSet{k} \to \sSet$ admits a right adjoint $\mathcal{N}^{(k)} \colon \sSet \to \mSet{k}$, defined, as usual, by the expression
\[
\mathcal{N}^{(k)}(Y)_{m_1,\dots,m_k} =
...
Recall that an \textit{$(n_1,\dots,n_k)$-shuffle} $\sigma$ is a permutation in $\Sigma_n$ satisfying
\begin{gather*}
