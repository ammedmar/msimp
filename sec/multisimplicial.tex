...
Composing with the diagonal functor
\[
\simplex^\op \xra{\diag}
...
Explicitly, for an $(m_1, \dots, m_k)$-multisimplex $x$ the Alexander--Whitney coproduct is given by
\[
\copr(x) =
...
\]
\anibal{What is $h$ in the exponent of the sign?}
...
%%% a version of the older version below:
%%%
% In order to write down an explicit formula for the coproduct 
% \[
% \chains(X) \to \chains(X) \ot \chains(X)
% \]
% we need the following definition.

% \begin{definition}
%     Let $X$ be a multisimplicial set and $x \in X_{n_1, \dots, n_k}$.
%     Given $0 \leq i_\ell \leq n_\ell$ for $\ell = 1, \dots, k$ the \textit{front $(i_1, \dots, i_k)$-face} of $x$ is the multisimplex
%     \[
%     x \rfloor_{(i_{1},\dots,i_{k})} =
%     X(F_{i_1}, \dots, F_{i_k})(x) \in X_{i_1,\dots,i_k}
%     \]
%     where
%     $F_{i_j} \colon [i_j] \to [n_j]$ is defined by $F_{i_j}(h)=h$, and the \textit{back $(i_1, \dots, i_k)$-face} of $x$ is the multisimplex
%     \[
%     \,_{(i_{1},\dots,i_{k})}\lfloor x =
%     X(B_{i_1},\dots,B_{i_k})(x) \in X_{i_1,\dots,i_k}
%     \]
%     where $B_{i_j} \colon [i_j] \to [n_j]$ is defined by $B_j(h) = h+n_j-i_j$. 
% \end{definition}

% If $x$ is as above then
% %proposition, theorem....
% \[
% \copr(x) = \sum_{j=1}^k \sum_
% {i_{j}=0}^{a_{j}} \;  
